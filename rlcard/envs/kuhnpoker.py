...
from rlcard.games.kuhnpoker import Game
...
class KuhnPokerEnv(Env):
    ...
    def __init__(self, config):
        ...
        with open(os.path.join(rlcard.__path__[0],
                  'games/kuhnpoker/card2index.json'), 'r') as file:
    ...
    def _get_legal_actions(self):
        ...
        Returns:
            encoded_action_list (list): return encoded legal action list
            (from str to int)
        '''
    ...
    def _extract_state(self, state):
        ...
        Returns:
            observation (list): combine the player's score and dealer's
            observable score for observation
        '''
        extracted_state = {}
    ...
    def get_perfect_information(self):
        ...
        Returns:
            (dict): All the perfect information of the current state
        '''
        state = {}
        state['chips'] = [
            self.game.players[i].in_chips
            for i in range(self.player_num)
        ]
        state['hand_cards'] = [
            self.game.players[i].hand.get_index()
            for i in range(self.player_num)
        ]
